...
function rankSections(multiResults: TMDBSearchResult[]): SectionName[] {
  const scores: Record<SectionName, number> = { Films: 0, Directors: 0, Actors: 0 }
...
function SearchSection<T>({ title, results, maxItems, renderItem }: SearchSectionProps<T>) {
  return (
    <div className="mt-2">
...
export default function QuickSearchModal({
  ...
  const [searchResult_Director, setSearchResult_Director] = useState<TMDBSearchResult[]>([])
  const [searchResult_Actor, setSearchResult_Actor] = useState<TMDBSearchResult[]>([])
  const [isSearching, setIsSearching] = useState(false)
  const [sectionOrder, setSectionOrder] = useState<SectionName[]>(["Films", "Directors", "Actors"])
          ...
          <div className="relative w-full min-w-[10rem] h-[2.5rem] md:h-[3rem] xl:h-[3.5rem] p-2 flex items-center gap-3">
            ...
            <input
              ...
              onKeyDown={(e: React.KeyboardEvent<HTMLInputElement>) => {
                if (e.key === "Enter") {
                  navigate({ to: "/films", state: { searchInputFromQuickSearch: e.currentTarget.value } })
                }
              }}
            />
            <button
                ...
                <SearchSection<TMDBSearchResult>
                  ...
                  renderItem={(person, key) => (
                    <SearchResultItem
                      ...
                      imageSrc={person.profile_path ? `${imgBaseUrl}${person.profile_path}` : "/picnotfound.jpg"}
