...
import InteractionConsole from "./Shared/film-interaction/InteractionConsole"
import PersonList from "./Shared/detail/PersonList"
import TrailerModal from "./Shared/detail/TrailerModal"
import Torrents from "./Shared/Films/Torrents"
import Subtitles from "./Shared/Films/Subtitles"
...
export default function FilmLanding() {
  ...
  const [backdropColor, setBackdropColor] = useState<[number, number, number]>([0, 0, 0])
  ...
  const [filmAwards, setFilmAwards] = useState<WikidataAwardsResponse | null>(null)
  ...
  useEffect(() => {
    ...
    try {
      ...
      backdrop.onload = () => {
        const domColor = getColorSync(backdrop).array() as [number, number, number]
