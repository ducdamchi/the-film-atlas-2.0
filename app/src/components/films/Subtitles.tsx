...
function SubtitleItemComponent({ subtitle }: SubtitleItemProps) {
...
export default function Subtitles({ subtitles }: SubtitlesProps) {
  const filtered =
        ...
        {displayed.length > 0 && (
          <div className="grid grid-cols-1 lg:grid-cols-2 2xl:grid-cols-3 gap-2">
            {displayed.map((subtitle, key) => (
              <SubtitleItemComponent key={key} subtitle={subtitle} />
            ))}
          </div>
        )}
      </div>
