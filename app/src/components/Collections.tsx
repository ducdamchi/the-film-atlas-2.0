...
export default function Collections() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (confirmed, vars) => {
      ...
      queryClient.setQueryData<AppCollection[]>(
        collectionsQueryOptions.queryKey,
        (old = []) =>
          old.map((c) => (c.id === vars.id ? confirmed : c)),
      );
      setNewCollectionId(null);
    },
  });
  ...
  function handleTogglePin(id: string): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      pinMutation.mutate({ id, pinned: next }, { onSuccess: () => resolve(), onError: reject });
    });
  }
  ...
  const descriptionMutation = useMutation({
    ...
    onMutate: async ({ id, description }) => {
      ...
      queryClient.setQueryData<AppCollection[]>(
        collectionsQueryOptions.queryKey,
        (old = []) =>
          old.map((c) => (c.id === id ? { ...c, description } : c)),
      );
      return { previous };
    },
    onError: (_err, _vars, context) => {
  ...
  const visibilityMutation = useMutation({
    ...
    onMutate: async ({ id, is_public }) => {
      ...
      queryClient.setQueryData<AppCollection[]>(
        collectionsQueryOptions.queryKey,
        (old = []) =>
          old.map((c) => (c.id === id ? { ...c, is_public } : c)),
      );
      return { previous };
    },
    onError: (_err, _vars, context) => {
